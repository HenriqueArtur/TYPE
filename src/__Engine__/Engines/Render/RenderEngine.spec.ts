...
describe("RenderEngine", () => {
  ...
  describe("setupScene", () => {
    it("should handle empty sprite entities", async () => {
      vi.spyOn(typeEngine.EntityEngine, "queryWithAny").mockReturnValue([]);
    ...
    it("should handle sprites when stage is initialized", async () => {
      ...
      vi.spyOn(testTypeEngine.EntityEngine, "queryWithAny").mockReturnValue([
        {
          entityId: "entity-1",
          components: {
      ...
      expect(testTypeEngine.EntityEngine.queryWithAny).toHaveBeenCalled();
    });
